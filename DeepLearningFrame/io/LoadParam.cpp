	...
	if(infile.is_open()){
		while(infile.good() && !infile.eof()){
			...
			if(varname == params_name[ALGORITHM]){
				multi_params[layer_order-1].algorithm = value;
				iter = 0;
				continue;
			}
			...
			if(varname == params_name[TRAINLABELS]){
				...
				if(filetype == "mat"){
					...
					if(values.size() == 4){
						...
						data_info.train_labels_info.rows = 0;
						data_info.train_labels_info.cols = 0;
					}
				}
				continue;
			}
			if(varname == params_name[TESTDATA]){
				...
				if(filetype == "mat"){
					...
					if(values.size() == 4){
						...
						data_info.test_data_info.rows = 0;
						data_info.test_data_info.cols = 0;
					}
				}
				continue;
			}
			if(varname == params_name[TESTLABELS]){
				...
				if(filetype == "mat"){
					...
					if(values.size() == 4){
						...
						data_info.test_labels_info.rows = 0;
						data_info.test_labels_info.cols = 0;
					}
				}
				continue;
			}
			if(varname == params_name[FINETUNEDATA]){
				...
				if(filetype == "mat"){
					...
					if(values.size() == 4){
						...
						data_info.finetune_data_info.rows = 0;
						data_info.finetune_data_info.cols = 0;
					}
				}
				continue;
			}
			if(varname == params_name[FINETUNELABELS]){
				...
				if(filetype == "mat"){
					...
					if(values.size() == 4){
						...
						data_info.finetune_labels_info.rows = 0;
						data_info.finetune_labels_info.cols = 0;
					}
				}
				continue;
			}
